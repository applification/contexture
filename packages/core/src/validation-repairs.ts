...
import type { Op, TypeUpdatePatch } from './ops';
...
export function repairForValidationError(
  schema: Schema,
  error: Pick<SemanticIssue, 'code' | 'path'>,
  ...
  if (error.code === 'convex_reserved_table_name' && type.kind === 'object') {
    return {
      ...
      op: {
        kind: 'update_type',
        name: type.name,
        patch: typePatch({ tableName: publicTableName(type) }),
      },
      focusTypeName: type.name,
    };
  }
  ...
  if (error.code === 'duplicate_convex_table_name' && type.kind === 'object') {
    return {
      ...
      op: {
        kind: 'update_type',
        name: type.name,
        patch: typePatch({ tableName: uniqueTableName(schema, type) }),
      },
      focusTypeName: type.name,
    };
  }
  ...
  if (error.code === 'duplicate_field_name' && type.kind === 'object') return null;
