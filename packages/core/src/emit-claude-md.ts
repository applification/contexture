...
on every IR save. To change the schema, use the Contexture CLI:

\`\`\`
bunx contexture list-types --json
bunx contexture get-type Post --json
bunx contexture add-field Post title '{"kind":"string"}'
bunx contexture validate --json
\`\`\`

The CLI updates \`packages/contexture/{{PROJECT_NAME}}.contexture.json\` and
re-emits the generated artefacts atomically. Hand-edit the IR only when the
CLI cannot express the change, then run \`bunx contexture validate --json\`.
