...
export const metadata: Metadata = {
  ...
  description: 'Ontograph brand identity guidelines: logo, colors, typography, voice, and components.',
}

/* ------------------------------------------------------------------ */
/*  Logo mark — three connected nodes in a triangle                   */
/* ------------------------------------------------------------------ */
function LogoMark({ className, variant = 'color' }: { className?: string; variant?: 'color' | 'mono-light' | 'mono-dark' }) {
  const primary = variant === 'mono-light' ? '#000' : variant === 'mono-dark' ? '#fff' : 'var(--primary)'
  const accent = variant === 'mono-light' ? '#000' : variant === 'mono-dark' ? '#fff' : 'var(--accent)'
  return (
    <svg className={className} viewBox="0 0 32 32" fill="none">
      <line x1="8" y1="24" x2="24" y2="24" stroke={primary} strokeWidth="1.5" strokeLinecap="round" />
      <line x1="8" y1="24" x2="16" y2="8" stroke={primary} strokeWidth="1.5" strokeLinecap="round" />
      <line x1="24" y1="24" x2="16" y2="8" stroke={primary} strokeWidth="1.5" strokeLinecap="round" />
...
/*  Type specimen                                                     */
/* ------------------------------------------------------------------ */
function TypeSpecimen({ font, name, weights, example }: { font: string; name: string; weights: string; example: string }) {
  return (
    <div className="rounded-xl border border-border/60 bg-card/30 p-8">
      <div className="flex items-baseline justify-between mb-4">
        <h4 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{name}</h4>
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-8 pt-32 pb-20">
        ...
        <header className="pb-16 border-b border-border/30">
          <p className="text-sm text-accent font-medium tracking-widest uppercase mb-4">Brand Guidelines</p>
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight mb-6">
            Ontograph Brand Identity
          </h1>
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            The visual and verbal identity standards for Ontograph. Use these guidelines to maintain consistency across all touchpoints — from marketing to product UI.
          </p>
        ...
        <Section id="logo" title="Logo">
          <p className="text-muted-foreground mb-8 max-w-2xl leading-relaxed">
            The Ontograph mark represents three connected nodes in a triangle — the fundamental ontology structure of subject-predicate-object triples. It should never be filled, rotated, or modified with effects.
          </p>
          ...
          <div className="rounded-xl border border-border/60 bg-card/30 p-8">
            <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Usage rules</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li>Always title-case: <strong className="text-foreground">Ontograph</strong>, never "ontograph" or "ONTOGRAPH".</li>
              <li>Minimum clear space around the mark: equal to the diameter of one node circle.</li>
              <li>Never stretch, rotate, add drop shadows, or apply gradients to the mark.</li>
              <li>For constrained contexts (favicon, 16px), use the mark alone without the wordmark.</li>
            </ul>
          </div>
        </Section>
        ...
        <Section id="colors" title="Color Palette">
          <p className="text-muted-foreground mb-8 max-w-2xl leading-relaxed">
            All colors use OKLCH for perceptual uniformity. The palette is built around deep indigo (intelligence, depth) with electric cyan as an accent (data, interactivity).
          </p>

          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Primary &amp; Accent</h3>
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Semantic</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-12">
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Backgrounds &amp; Surfaces</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-12">
            <Swatch name="Background (light)" value="oklch(0.98 0.005 270)" token="--background" />
            <Swatch name="Background (dark)" value="oklch(0.14 0.02 270)" token="--background" dark />
            <Swatch name="Card (light)" value="oklch(1 0 0)" token="--card" />
            <Swatch name="Card (dark)" value="oklch(0.18 0.02 270)" token="--card" dark />
          </div>

          <div className="rounded-xl border border-border/60 bg-card/30 p-8">
            <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Neutrals</h3>
            <p className="text-sm text-muted-foreground">
              All neutral grays carry a subtle cool tint at 270 hue angle (indigo family). This creates a cohesive feel without being obviously tinted. Dark mode is the <strong className="text-foreground">default</strong> theme — the graph canvas renders best on dark backgrounds.
            </p>
          </div>
        </Section>
        ...
        <Section id="typography" title="Typography">
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Type scale</h3>
          <div className="rounded-xl border border-border/60 bg-card/30 p-8 space-y-6">
            <div className="flex items-baseline gap-6">
              <span className="text-xs text-muted-foreground font-mono w-20 shrink-0">text-2xl</span>
            ...
            <div className="flex items-baseline gap-6">
              <span className="text-xs text-muted-foreground font-mono w-20 shrink-0">text-base</span>
            ...
            <div className="flex items-baseline gap-6">
              ...
              <span className="text-sm text-muted-foreground">Labels, descriptions, secondary text</span>
            </div>
            <div className="flex items-baseline gap-6">
        ...
        <Section id="voice" title="Voice & Tone">
          <p className="text-muted-foreground mb-8 max-w-2xl leading-relaxed">
            Ontograph speaks to engineers who ship products — precise, confident, and approachable. Never academic, never dismissive.
          </p>
          ...
          <div className="grid sm:grid-cols-2 gap-6 mb-10">
            <div className="rounded-xl border border-border/60 bg-card/30 p-8">
              ...
              <ul className="space-y-3 text-sm text-muted-foreground">
                <li><strong className="text-foreground">Precise</strong> — Use correct terminology (ontology, OWL, RDF) without being academic.</li>
                <li><strong className="text-foreground">Confident</strong> — State what we do directly, no hedging.</li>
                <li><strong className="text-foreground">Approachable</strong> — Make ontology engineering feel accessible.</li>
                <li><strong className="text-foreground">Builder-oriented</strong> — Speak to people who ship, not who publish.</li>
              </ul>
            </div>
            <div className="rounded-xl border border-border/60 bg-card/30 p-8">
              <h3 className="text-sm font-medium text-destructive uppercase tracking-wider mb-4">Don&apos;t</h3>
              <ul className="space-y-3 text-sm text-muted-foreground">
        ...
        <Section id="components" title="Components">
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Buttons</h3>
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Cards</h3>
          <div className="grid sm:grid-cols-2 gap-6 mb-10">
            <div className="rounded-xl border border-border/60 bg-card/50 p-8 hover:border-primary/30 transition-colors">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">Cards use border-border/60 with bg-card/50. Hover state shifts border toward primary/30.</p>
            </div>
            <div className="rounded-xl border border-border/60 bg-card/30 p-8 font-mono text-sm">
              <div className="text-muted-foreground/70 text-xs uppercase tracking-wide mb-2">Terminal block</div>
          ...
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Badges &amp; pills</h3>
          ...
          <div className="rounded-xl border border-border/60 bg-card/30 p-8">
            <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Design system stack</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li><strong className="text-foreground">shadcn/ui</strong> — component primitives (Button, Dialog, Popover, etc.)</li>
              <li><strong className="text-foreground">Tailwind CSS v4</strong> — utility-first styling with OKLCH color tokens</li>
              <li><strong className="text-foreground">Geist</strong> — sans + mono variable fonts</li>
              <li><strong className="text-foreground">Lucide</strong> — icon set, consistent stroke style</li>
              <li><strong className="text-foreground">Framer Motion</strong> — complex animations; CSS transitions for simple states</li>
            </ul>
          </div>
        </Section>
        ...
        <Section id="motion" title="Motion">
          <p className="text-muted-foreground mb-8 max-w-2xl leading-relaxed">
            Motion is purposeful and restrained. Every animation serves a function — guiding attention, confirming actions, or smoothing transitions.
          </p>

          <div className="grid sm:grid-cols-2 gap-6 mb-10">
            <div className="rounded-xl border border-border/60 bg-card/30 p-8">
              <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Duration scale</h3>
              <div className="space-y-4">
                {[
                  { label: 'Micro', duration: '150ms', easing: 'ease-out', desc: 'Hover, focus' },
                  { label: 'Standard', duration: '200ms', easing: 'ease-in-out', desc: 'Panels, modals' },
                  { label: 'Emphasis', duration: '300ms', easing: 'spring', desc: 'Page transitions' },
                  ...
                  <div key={m.label} className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-mono text-muted-foreground w-16">{m.duration}</span>
            ...
            <div className="rounded-xl border border-border/60 bg-card/30 p-8">
              <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Principles</h3>
              <ul className="space-y-3 text-sm text-muted-foreground">
                <li><strong className="text-foreground">Purposeful</strong> — every animation serves user comprehension.</li>
                <li><strong className="text-foreground">Quick</strong> — no animation should feel like waiting.</li>
                <li><strong className="text-foreground">Consistent</strong> — same pattern = same duration/easing.</li>
                <li><strong className="text-foreground">Reducible</strong> — respect prefers-reduced-motion.</li>
              </ul>
            </div>
          </div>
        </Section>
      ...
      <footer className="border-t border-border/30 py-10 px-8">
        <div className="max-w-5xl mx-auto flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-6">
            <a href="https://github.com/DaveHudson/Ontograph" className="hover:text-foreground transition-colors">GitHub</a>
            <a href="https://github.com/DaveHudson/Ontograph/releases" className="hover:text-foreground transition-colors">Changelog</a>
          </div>
        </div>
      </footer>
    </div>
