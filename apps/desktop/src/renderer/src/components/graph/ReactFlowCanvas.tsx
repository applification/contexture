...
function GraphFlow(): React.JSX.Element {
  ...
  useEffect(() => {
    const { nodes: newNodes, edges: newEdges } = ontologyToReactFlowElements(
      ontology,
      validationErrors,
    );
          ...
          items: [
            {
              ...
              action: () => {
                ...
                const parent = Array.from(ontology.classes.values()).find(
                  (c) =>
                    c.label?.toLowerCase() === parentLabel.toLowerCase() || c.uri === parentLabel,
                );
                if (!parent) {
                  alert(`Class "${parentLabel}" not found`);
                  return;
                }
          ...
          },
          {
            ...
            action: () => {
              ...
              const target = Array.from(ontology.classes.values()).find(
                (c) =>
                  c.label?.toLowerCase() === targetLabel.toLowerCase() || c.uri === targetLabel,
              );
              if (!target) {
                alert(`Class "${targetLabel}" not found`);
                return;
              }
        ...
        items: [
          ...
          },
          {
            ...
            action: () => {
              ...
              const findClass = (q: string) =>
                Array.from(ontology.classes.values()).find(
                  (c) => c.label?.toLowerCase() === q.toLowerCase() || c.uri === q,
                );
              const domainCls = findClass(domainLabel);
              const rangeCls = findClass(rangeLabel);
              if (!domainCls) {
                alert(`Domain class "${domainLabel}" not found`);
                return;
              }
              if (!rangeCls) {
                alert(`Range class "${rangeLabel}" not found`);
                return;
              }
              const uri = `${baseNsRef.current}${name.replace(/\s+/g, '')}`;
              addObjectProperty(uri, {
                label: name,
                domain: [domainCls.uri],
                range: [rangeCls.uri],
              });
            },
          },
  ...
  return (
    <div className="relative w-full h-full">
      <ReactFlow
        ...
        onEdgeContextMenu={
          handleEdgeContextMenu as unknown as Parameters<typeof ReactFlow>[0]['onEdgeContextMenu']
        }
