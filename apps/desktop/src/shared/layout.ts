 ...
 * `version: '1'`; if we read a file whose version we don't recognise we discard
 * it and return defaults with a warning, rather than throwing. The IR itself is
 * the source of truth — nothing here should be allowed to block a user from
 * opening their schema.
 */
...
/**
 * Lockstep rename: when a `TypeDef` is renamed, update the layout key (and any
 * group members list) to match. Returns a new layout; does not mutate. If
 * `from` is absent, the layout is returned unchanged.
 */
