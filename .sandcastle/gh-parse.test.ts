import { describe, expect, test } from "bun:test";
import {
  extractClosingNumbers,
  IssueListSchema,
  IssueSnapshotSchema,
  PRListSchema,
  prListToClosing,
} from "./gh-parse";

describe("extractClosingNumbers", () => {
  test("matches Closes / Fixes / Resolves with optional past tense, case-insensitive", () => {
    const body = `
      This PR Closes #1, fixes #2, resolved #3, and FIXED #4.
      Also closes #5.
    `;
    expect(extractClosingNumbers(body).sort((a, b) => a - b)).toEqual([1, 2, 3, 4, 5]);
  });

  test("returns an empty array when body is null", () => {
    expect(extractClosingNumbers(null)).toEqual([]);
  });

  test("returns an empty array when body has no closing markers", () => {
    expect(extractClosingNumbers("Just some prose mentioning #99 in passing.")).toEqual([]);
  });

  test("dedupes repeated mentions", () => {
    expect(extractClosingNumbers("Closes #7. Fixes #7. Resolves #7.")).toEqual([7]);
  });

  test("ignores numbers that aren't preceded by a closing keyword", () => {
    expect(extractClosingNumbers("See #42 for context. Closes #43.")).toEqual([43]);
  });
});

describe("IssueListSchema", () => {
  test("accepts well-formed gh issue list output and flattens labels", () => {
    const raw = [
      { number: 1, title: "First", state: "OPEN", labels: [{ name: "Sandcastle" }] },
      { number: 2, title: "Second", state: "open", labels: [] },
    ];
    expect(IssueListSchema.parse(raw)).toEqual([
      { number: 1, title: "First", state: "open", labels: ["Sandcastle"] },
      { number: 2, title: "Second", state: "open", labels: [] },
    ]);
  });

  test("rejects negative issue numbers", () => {
    expect(() =>
      IssueListSchema.parse([{ number: -1, title: "x", state: "open", labels: [] }]),
    ).toThrow();
  });

  test("rejects missing labels field", () => {
    expect(() =>
      IssueListSchema.parse([{ number: 1, title: "x", state: "open" }]),
    ).toThrow();
  });

  test("rejects missing state field", () => {
    expect(() =>
      IssueListSchema.parse([{ number: 1, title: "x", labels: [] }]),
    ).toThrow();
  });
});

describe("PRListSchema", () => {
  test("accepts a null body (gh returns null when PR has no description)", () => {
    expect(PRListSchema.parse([{ number: 1, body: null }])).toEqual([{ number: 1, body: null }]);
  });

  test("accepts a string body", () => {
    expect(PRListSchema.parse([{ number: 1, body: "Closes #2" }])).toEqual([
      { number: 1, body: "Closes #2" },
    ]);
  });

  test("rejects a non-string non-null body", () => {
    expect(() => PRListSchema.parse([{ number: 1, body: 42 }])).toThrow();
  });
});

describe("IssueSnapshotSchema", () => {
  const base = { number: 1, title: "x", labels: [{ name: "Sandcastle" }] };

  test("normalises uppercase state to lowercase and flattens labels", () => {
    expect(IssueSnapshotSchema.parse({ ...base, state: "OPEN" })).toEqual({
      number: 1,
      title: "x",
      state: "open",
      labels: ["Sandcastle"],
    });
    expect(IssueSnapshotSchema.parse({ ...base, state: "CLOSED" })).toEqual({
      number: 1,
      title: "x",
      state: "closed",
      labels: ["Sandcastle"],
    });
  });

  test("accepts already-lowercase state", () => {
    expect(IssueSnapshotSchema.parse({ ...base, state: "open" })).toEqual({
      number: 1,
      title: "x",
      state: "open",
      labels: ["Sandcastle"],
    });
  });

  test("rejects unknown state values", () => {
    expect(() => IssueSnapshotSchema.parse({ ...base, state: "merged" })).toThrow();
  });
});

describe("prListToClosing", () => {
  test("maps validated PRs to (pr, closes[]) tuples", () => {
    const prs = PRListSchema.parse([
      { number: 100, body: "Closes #1, fixes #2." },
      { number: 101, body: null },
      { number: 102, body: "no closing markers here" },
    ]);
    expect(prListToClosing(prs)).toEqual([
      { pr: 100, closes: [1, 2] },
      { pr: 101, closes: [] },
      { pr: 102, closes: [] },
    ]);
  });
});
